...
The overview page includes a built-in summarizer (`lib/ai/summarizer.ts`):

- **Metrics** (retail units, contact rate, SLA, revenue) are computed in `lib/metrics/` and shown as highlight chips.
- **Narrative** is assembled by a rule-based engine — no external LLM or API key.
- Insight candidates from `lib/insights.ts` are scored by severity, revenue impact, and period relevance, then woven into headline, body, and priority bullets.
- Theme-specific sentences use verified KPIs only; numbers in prose always match the metric layer.
